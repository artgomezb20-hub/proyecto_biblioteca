...
RANGOS_SHEET = None  # No hay nombre de hoja específico
...
def load_dataframe():
    """Carga el catálogo de libros desde XLSX o JSON."""
    global _df_cache
    _ensure_data_dir()
    ...
    if XLSX_FILE.exists():
        ...
        df = pd.DataFrame(columns=[
            "id", "titulo", "autor", "isbn", "categoria", "anio",
            "paginas", "descripcion", "portada_url", "sala",
            "estante", "pos_x", "pos_y"
        ])
...
def _ensure_rangos_df():
    """Carga el DataFrame de rangos Dewey + Cutter."""
    global _df_rangos
...
def _ensure_mapa_df():
    """Carga el CSV del mapa de estanterías (si existe)."""
    global _df_mapa
